...
const Suppliers = () => {
    ...
    const fetchData = async () => {
        ...
        catch (e) { toast.error('Failed to fetch suppliers'); }
        finally { setLoading(false); }
    };
    useEffect(() => { fetchData(); }, []);

    const openAdd = () => { setEditing(null); setForm({ name: '', email: '', phone: '', address: '' }); setIsModalOpen(true); };
    const openEdit = (s) => { setEditing(s); setForm({ name: s.name, email: s.email || '', phone: s.phone || '', address: s.address || '' }); setIsModalOpen(true); };
    ...
    const handleDelete = async () => {
        try { await api.delete(`/suppliers/${deleteId}`); toast.success('Supplier deleted'); fetchData(); }
        catch (e) { toast.error('Delete failed'); }
    };

    const columns = [
        { key: 'name', header: 'Supplier', sortable: true, accessor: 'name', render: (r) => (
            <div>
    ...
    return (
        <div className="space-y-6 animate-slide-up">
            <div className="flex items-center justify-between">
                <div><h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Suppliers</h1><p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{suppliers.length} suppliers</p></div>
                <button onClick={openAdd} className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2.5 rounded-lg flex items-center gap-2 text-sm font-medium transition-colors shadow-sm"><Plus className="w-4 h-4" /> Add Supplier</button>
            </div>
            <DataTable columns={columns} data={suppliers} loading={loading} searchPlaceholder="Search suppliers..." emptyTitle="No suppliers" emptyDescription="Add your first supplier." emptyIcon={Truck} />
            ...
            <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Supplier" message="Products from this supplier will remain but be unlinked." />
        </div>
    );
};
